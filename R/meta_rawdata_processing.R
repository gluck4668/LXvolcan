...
meta_rawdata_processing <- function(data_file){

# 判断数据文件csv或者xlsx，读文件
if(grepl("csv",data_file,ignore.case = TRUE))
    df0 <- read.csv(data_file) else
      df0 <- read.xlsx(data_file)

df <- df0[-c(1:2),]
names(df) <- df0[2,]

title_text <- names(df0)

nor <- grep("Normalised",title_text,ignore.case = TRUE) %>% as.numeric()
raw <- grep("Raw",title_text,ignore.case = TRUE)-1

# 去掉空格方法：
groups <- df0[1,c(nor:raw)] %>% trimws() %>% as.character() %>% na.omit() # 转换为字符串
c(groups)=="" #查看哪些是空格
!c(groups)=="" #不是空格
group_names <- groups[!c(groups)==""] # 只列出不是空格的字符串

# 可以整合成一条代码
group_names <- df0[1,c(nor:raw)] %>% trimws() %>% as.character() %>% na.omit() %>% .[!c(.)==""]
