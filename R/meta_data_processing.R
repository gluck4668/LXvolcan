...
# 选出做火山图需要的数据
  meta <- df[,c(1:5)]
  colnames(meta) <- c("ID","FC","High","pvalue","qvalue")
...
# 标注down, up 和 not sig
  if(is.null(FC)==TRUE)
  {meta$type <- ifelse(meta$log2FC>0 & meta$pvalue<0.05,"Up",
                     ifelse(meta$log2FC<0 & meta$pvalue<0.05,"Down","Not sig")) } else
                     {meta$type <- ifelse(meta$log2FC>=log2(FC) & meta$pvalue<0.05,"Up",
                                        ifelse(meta$log2FC<=-log2(FC) & meta$pvalue<0.05,"Down","Not sig"))}

  change_n <- table(meta$type) %>% data.frame()
  ...
  meta$changes <- ifelse(meta$type=="Up",UP,
                       ifelse(meta$type=="Down",Down,No))
  data <- meta
