...
#------判断是否有HMDB ID，如果没有，则用代谢名称-------------
hmdb <- df$`Accepted Compound ID`
is.hmdb <- grepl("HMDB",hmdb,ignore.case = T) %>% table()
if("TRUE" %in% names(is.hmdb))
    lst1 <- grep("Accepted",names(df),ignore.case = TRUE)[1] else
      lst1 <- grep("Accepted",names(df),ignore.case = TRUE)[2]
...
meta_set <- df[,c(lst1,lst2,lst3,lst4,lst5,lst6)]
colnames(meta_set)[1] <- c("ID")

result <- meta_set[!meta_set$ID=="",] # 支掉代谢物名称是空格的行
