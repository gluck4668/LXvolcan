...
# 判断数据类型
df_type <- c("gene") # 默认是 gene

# 接下来，判断是否含代谢数据特别标示“Highest”，如果有，则df_type替换为"metabolite"
is.meta <- grepl("Highest",file_type[2,],ignore.case = T) %>% table()
if("TRUE" %in% names(is.meta))
  df_type <- c("metabolite")

is.meta <- grepl("Highest",names(file_type),ignore.case = T) %>% table()
if("TRUE" %in% names(is.meta))
  df_type <- c("metabolite")
...
if (df_type == "gene")
      df <- gene_data_processing(data_file) else
        {is_rawdata <- grepl("Neutral",file_type[2,],ignore.case = TRUE) %>% table()
         if("TRUE" %in% names(is_rawdata))
           {raw_data <-rawdata_processing(file_type)
             id_names <- raw_data$id_names
             data_meta <- raw_data$meta_set} else
...
df_order <- data[data[,7]>1.30103,] # 筛选出 > -log10(0.05)，即p<0.05
...
# 根据df_type的类型，进行相应的数据处理
if(df_type == "metabolite")
  {if("TRUE" %in% names(is_rawdata)){
    colnames(id_names) <- c("ID","Metabolites")
    duplicated(id_names$ID) %>% table()
    id_names <- distinct(id_names,ID,.keep_all = T)
    df_lst <- dplyr::inner_join(id_names,df_lst,"ID")}
  }
