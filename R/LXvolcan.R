...
# 标注down, up 和 not sig
 ...
 #########################################################
  ...
  change_type <- data$changes[!duplicated(data$changes)]
  if(length(grep("NA",change_type,ignore.case = T))>0)
    print("The pvalue and/or FC were abnormal in the data file. Please check it.")
