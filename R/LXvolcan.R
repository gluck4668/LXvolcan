...
#'
#'
LXvolcan <- function(data_type,data_file,FC){
...
# 判断数据文件data_file与类型data_type是否匹配
if (trimws(tolower(data_type))=="gene")
  {df_read <- read.xlsx(data_file)
   is_meta <- grepl("HMDB",df_read,ignore.case = T) %>% table()
  if("TRUE" %in% names(is_meta))
  {rm(df_read)
    stop(paste0("The data file maybe about metabolites, but the data_type is ",data_type,
                ". Please check it."))} else
                  rm(df_read)
  data <- gene_data_processing(data_file)
  df_type <- c("gene")
  }

if (trimws(tolower(data_type))=="metabolite")
    {df_read <- read.xlsx(data_file)
     is_meta <- grepl("HMDB",df_read,ignore.case = T) %>% table()
     if(!"TRUE" %in% names(is_meta))
      {rm(df_read)
      stop(paste0("The data file maybe about gene, but the data_type is ",data_type,
                  ". Please check it."))} else
                    rm(df_read)
     data <- meta_data_processing(data_file)
...
# 标注down, up 和 not sig
  ...
  df_group <- read.xlsx(data_file)

  group_name <- colnames(df_group) %>%
