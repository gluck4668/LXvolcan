...
pack <- c("ggplot2","openxlsx","stringr","plyr","psych","dplyr",
          "ggrepel","patchwork","raster","png","janitor","purrr","readr")
...
#----- 数据处理---------------
file_type <- str_extract(trimws(data_file),"(?<=[.]).*")  # 判断数据文件类型
if(tolower(file_type)=="txt")
  df_set <- read_table(data_file) else  # txt用read.table，其他类型用read.xlsx, read.csv等
   {read.file <- paste0("read.",file_type,"(data_file)") # 应用相应的read
   df_set <- eval(str2expression(read.file))}

# 根据data_type的类型，进行相应的数据处理
if(tolower(trimws(data_type))=="gene")
  df <- gene_data_processing(df_set)

if(tolower(trimws(data_type))=="protein")
  df <- protein_data_processing(df_set)

if(tolower(trimws(data_type))=="metabolite")
    df <- meta_data_processing(df_set)

if(tolower(trimws(data_type))=="metabolite_raw_data")
  df <- meta_rawdata_processing(df_set)
#----------------------------
