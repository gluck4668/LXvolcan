...
if(any(grepl("ENS",df0[,1],ignore.case = T))) #判断是否是ENS ID, 如果是则转换为UNIPROT ID
  {meta_id <- bitr(geneID = c("ENSP00000413622",df0$prot_id),fromType = "ENSEMBLPROT",
  ...
  if(!any(grepl("ENSP00000413622",df$prot_id,ignore.case = T)))
    prot_list <-prot_list[-grep("ENSP00000413622",prot_list[,1],ignore.case = T),]
  }

 #----------物种是小鼠----------------------
  meta_id <- bitr(geneID = c("ENSMUSP00000030792",df0$prot_id),fromType = "ENSEMBLPROT",
  ...
  {prot_list <- meta_id
  names(prot_list) <- c("prot_id","Uniprot_id")
  if(!any(grepl("ENSMUSP00000030792",df0$prot_id,ignore.case = T)))
    prot_list <-prot_list[-grep("ENSMUSP00000030792",prot_list[,1],ignore.case = T),]
  }

 #----------物种是大舅----------------------
  meta_id <- bitr(geneID =c("ENSRNOP00000029808",df0$prot_id),fromType = "ENSEMBLPROT",
  ...
  if(nrow(meta_id)>1)
    {prot_list <- meta_id
     names(prot_list) <- c("prot_id","Uniprot_id")
       if(!any(grepl("ENSRNOP00000029808",df0$prot_id,ignore.case = T)))
       prot_list <-prot_list[-grep("ENSRNOP00000029808",prot_list[,1],ignore.case = T),]
     }
